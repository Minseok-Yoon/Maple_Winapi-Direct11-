...
void CDamage::Init(int _iDamage, Vector2& _vPosition, bool _bIsCri, CDamageManager* _pManager, int i)
{
    fDelay = i * 0.1f;
    bEnable = false;
    fExistTime = 0.f;
    ...
    Vector2 tFirstPos = Vector2(_vPosition.x - (float)iLength * 20.f / 2.f, _vPosition.y - 100.f - (i - 1) * 26.f);
    ...
    for (int j = 0; j < iLength; ++j)
    {
        ...
        Vector2 tPos = Vector2(tFirstPos.x + j * 20.f, tFirstPos.y + 8.f - (float)(j % 2));
...
void CDamage::Render(HDC _dc)
{
    ...
    //        Vector2 position = m_vecPos[i];

    //        if (pTexture)
    //        {
    //            // 텍스처를 position 위치에 렌더링
    //            int iWidth = (int)pTexture->GetWidth();
    //            int iHeight = (int)pTexture->GetHeight();
