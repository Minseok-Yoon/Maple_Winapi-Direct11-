...
bool CVertexBuffer::Create(const vector<Vertex>& _vecVertexes)
{
	...
	desc.ByteWidth = sizeof(Vertex) * (UINT)_vecVertexes.size();			// 버퍼의 전체 크기를 설정. Vertex의 크기와 개수를 곱해 총 메모리 용량을 지정
	desc.BindFlags = D3D11_BIND_FLAG::D3D11_BIND_VERTEX_BUFFER;				// 이 버퍼가 VertexBuffer로 사용될 것임을 지정. VertexBuffer로 바인드
	desc.Usage = D3D11_USAGE::D3D11_USAGE_DYNAMIC;							// 이 버퍼의 사용 방법을 지정. DYNAMIC으로 설정하여 CPU에서 자주 업데이트를 수행
	desc.CPUAccessFlags = D3D11_CPU_ACCESS_FLAG::D3D11_CPU_ACCESS_WRITE;	// CPU에서 버퍼에 접근하여 데이터를 쓸 수 있도록 설정. 이를 통해 Vertex 데이터를 CPU가 수정할 수 있게 함
...
void CVertexBuffer::Bind()
{
	...
	{
		assert(NULL, "버텍스 버퍼를 세팅 할 수 없습니다.");
	}*/
